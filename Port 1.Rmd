...
```{r}
#Renaming the Subject coloumns
colnames(demo_train)[colnames(demo_train)=="Child.ID"] <- "ID"
colnames(lu_train)[colnames(lu_train)=="SUBJ"] <- "ID"
colnames(token_train)[colnames(token_train)=="SUBJ"] <- "ID"

#Renaming the visit coloumn
...
#removing letters from the visit coloumn
...
```{r}
#removing punctuations from ID coloumn
...
#Choosing the relevant coloumns
sub_demo <- select(demo_train, ID, VISIT, Diagnosis, Ethnicity, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)
sub_token<- select(token_train, ID, VISIT, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
sub_lu <- select(lu_train, ID, VISIT, MOT_MLU, CHI_MLU)
```
...
```{r}
#visit1 <- filter(df, VISIT==1)
#visit1 <- data.frame(visit1$ID, visit1$ADOS, visit1$MullenRaw, visit1$ExpressiveLangRaw, visit1$Socialization)


#creating dataframe for visit 1 with relevant coloumns
visit1 <- subset(df, VISIT == 1, select = c(ID, ADOS, MullenRaw, ExpressiveLangRaw, Socialization))
...
#Removing Visit coloumn
df <- df %>% 
  select(-VISIT)

#Merge the two df's
new_df <- merge(df, visit1)
...
# 2. MLU below 1.5 at first visit
MLU15 <- df %>% 
  filter(VISIT == 1 & CHI_MLU < 1.5)

# 3, not completed all trials 

NAs <- df %>%
  filter(is.na(ADOS)|is.na(MullenRaw)| is.na(ExpressiveLangRaw))
